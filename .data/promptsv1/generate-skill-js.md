...
> **Base instructions**: The shared base instructions (structure, quality gates, execution order) are already included above this prompt — do NOT attempt to read \_base-skill-generator.md from disk; it does not exist in the target project. Below are JS/TS-specific overrides.
...
12. **Session Protocols** (≤20 lines) — interaction modes with JS/TS-specific detection signals (e.g., "what is this type" for Teaching, "another component like X" for Efficient, "TypeError" for Diagnostic), plus self-learning via LEARNED.md; deeper guidance (proficiency calibration, anti-dependency nudges) goes to references/ai-interaction-guide.md — never into SKILL.md
