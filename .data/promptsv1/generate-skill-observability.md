...
> **Base instructions**: The shared base instructions (structure, quality gates, execution order) are already included above this prompt — do NOT attempt to read \_base-skill-generator.md from disk; it does not exist in the target project. Below are observability-specific overrides.
...
3. **Architecture** — instrumentation architecture diagram, telemetry pipeline flow, signal relationships (brief prose/table only — do NOT render an ASCII directory tree or multi-line structure diagram in SKILL.md; put any full structure map in `references/architecture-guide.md`, per the ≤5-line code-block gate)
...
12. **Session Protocols** (≤20 lines) — interaction modes with observability-specific detection signals (e.g., "alert firing" for Diagnostic, "add metrics to this service" for Efficient, "what is an error budget" for Teaching), plus self-learning via LEARNED.md; deeper guidance (proficiency calibration, anti-dependency nudges) goes to references/ai-interaction-guide.md — never into SKILL.md
