...
3. **Architecture** — performance-relevant architecture diagram, bottleneck zones, data flow with latency annotations (brief prose/table only — do NOT render an ASCII directory tree or multi-line structure diagram in SKILL.md; put any full structure map in `references/architecture-guide.md`, per the ≤5-line code-block gate)
