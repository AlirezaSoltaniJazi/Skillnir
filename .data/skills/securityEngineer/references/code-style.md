...
| Validation functions     | `validate_`/`is_` prefix     | `is_valid_gchat_webhook()`, `is_valid_slack_webhook()` (`notifications/providers.py`) |
| Sanitization functions   | `sanitize_`/`escape_` prefix | `html.escape()` (stdlib, used throughout landing-page generators)                     |
