...
- **Skill scope picker now categorized** -- new `SCOPE_CATEGORIES` constant in [src/skillnir/skill_generator.py](src/skillnir/skill_generator.py) groups all 32 scopes into 5 domains: **Engineering Roles** (10), **Quality & Testing** (9), **Architecture & Platform** (9, includes new `devops-engineer`), **Design** (1: `ui-ux-designer`), **Business & People** (3: `project-manager`, `financial-manager`, `hr-manager`). The CLI questionary picker ([src/skillnir/cli.py](src/skillnir/cli.py) `_generate_skill()`) now uses `questionary.Separator("── <category> ──")` between groups; the UI dropdown ([src/skillnir/ui/pages/generate_skill.py](src/skillnir/ui/pages/generate_skill.py)) prefixes each option with `<category> · <label>` and widens the select from `w-64` to `w-96` to fit the longer labels. New tests (`TestScopeCategories`) enforce that every scope belongs to exactly one category — drift between `SKILL_SCOPES` and `SCOPE_CATEGORIES` is now caught by the test suite.
