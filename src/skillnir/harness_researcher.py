"""Harness-engineering research pipeline: search the web for AI agent / LLM harness-engineering news, deep-read articles, summarize, generate landing page.

Sibling of :mod:`skillnir.researcher` (AI engineering),
:mod:`skillnir.software_researcher` (software engineering), and
:mod:`skillnir.testing_researcher` (testing & QA). Same data model,
dedup, HTML / markdown writers, landing-page builder, and subprocess
flow — only the topic catalog, source allowlist, output directory, and
prompt focus differ. "Harness engineering" here means the scaffolding
that turns an LLM into a reliable agent: the agent loop, tool calling,
context management, orchestration, memory, evals, guardrails, and the
observability/reliability around them. Designed so the domains can
...
@dataclass
class Article:
    """A single harness-engineering research article with deep summary."""
    ...
    topic: str  # agent-loops, tool-use, context-engineering, evals, etc.
...
@dataclass
class HarnessResearchResult:
    """Result of a harness-engineering research pipeline run."""
...
# ---------------------------------------------------------------------------
# Topics — 14 AI agent / LLM harness-engineering topic queries
# ---------------------------------------------------------------------------

RESEARCH_TOPICS: tuple[str, ...] = (
    "LLM agent loops control flow ReAct plan-and-execute agentic workflows orchestration 2025 2026",
    "LLM tool use function calling tool design JSON schemas structured outputs parallel tools 2025 2026",
    "context engineering context window management compaction prompt caching long-context retrieval 2025 2026",
    "multi-agent systems orchestration agent delegation supervisor handoff swarm collaboration 2025 2026",
    "AI agent memory retrieval augmented generation RAG vector search episodic semantic memory 2025 2026",
    "LLM agent evaluation evals benchmarks task success rate LLM-as-judge trajectory scoring 2025 2026",
    "LLM agent observability tracing logging OpenTelemetry token usage cost debugging spans 2025 2026",
    "LLM guardrails safety prompt injection jailbreak output validation content filtering alignment 2025 2026",
    "prompt engineering instruction design system prompts few-shot chain-of-thought decomposition 2025 2026",
    "Model Context Protocol MCP tool servers agent tool protocols interoperability connectors 2025 2026",
    "LLM code execution sandboxing secure tool execution containers isolation computer use 2025 2026",
    "LLM streaming responses agent UX interruption human-in-the-loop approval steering UI 2025 2026",
    "LLM inference cost latency optimization prompt caching batching model routing distillation 2025 2026",
    "LLM agent reliability error handling retries recovery self-correction fallback idempotency 2025 2026",
)

TOPIC_LABELS: dict[str, str] = {
    "agent-loops": "Agent Loops & Control Flow",
    "tool-use": "Tool & Function Calling",
    "context-engineering": "Context Engineering",
    "multi-agent": "Multi-Agent Orchestration",
    "memory-rag": "Memory & Retrieval",
    "evals": "Agent Evaluation",
    "observability": "Tracing & Observability",
    "guardrails": "Guardrails & Safety",
    "prompt-engineering": "Prompt & Instruction Engineering",
    "mcp": "MCP & Tool Protocols",
    "sandboxing": "Code Execution & Sandboxing",
    "streaming-ux": "Streaming & Agent UX",
    "cost-latency": "Cost & Latency Optimization",
    "reliability": "Reliability & Recovery",
}


# ---------------------------------------------------------------------------
# Preferred sources — high-signal AI agent / LLM engineering domains
# ---------------------------------------------------------------------------
...
SOURCE_FILTERS: dict[str, str] = {
    "labs": "AI Labs (Anthropic, OpenAI, Google DeepMind, Meta AI)",
    "frameworks": "Agent Frameworks (LangChain, LlamaIndex, CrewAI, AutoGen, MCP)",
    "practitioners": "Practitioners (Lilian Weng, Eugene Yan, Simon Willison, Hamel, Raschka)",
    "evals-observability": "Evals & Observability (Braintrust, Weights & Biases)",
    "vector-memory": "Vector & Memory (Pinecone, Weaviate, Hugging Face)",
    "research-pubs": "Research & Publications (arXiv, Distill, The Gradient)",
    "communities": "Communities & Newsletters (Latent Space, Interconnects, TDS)",
}
...
# ---------------------------------------------------------------------------
# Topic colors for the landing page chips (kept distinct from sibling pipelines)
# ---------------------------------------------------------------------------

_TOPIC_COLORS: dict[str, str] = {
    "agent-loops": "#1d4ed8",
    "tool-use": "#7c3aed",
    "context-engineering": "#059669",
    "multi-agent": "#0891b2",
    "memory-rag": "#db2777",
    "evals": "#d97706",
...
_HARNESS_RESEARCH_PROMPT = """\
You are a deep research assistant focused on **AI agent / LLM harness engineering** — \
the scaffolding that turns a language model into a reliable agent (agent loops, tool \
calling, context management, orchestration, memory, evals, guardrails, and the \
observability and reliability around them). Your task is to search the internet for the \
latest articles, papers, framework releases, case studies, and post-mortems on these \
topics, then deeply read each one and extract the most important, actionable insights \
for AI engineers, agent developers, and ML platform engineers building LLM-powered \
...
1. **Search** for recent, high-quality articles (AI-lab engineering posts, framework docs and blogs, agent design essays, eval write-ups, production post-mortems, research papers)
2. **Deep-read** each article — do not just paraphrase the title
3. **Extract** 3-5 KEY INSIGHTS that are actionable and specific (e.g., a concrete agent-loop pattern, a tool-schema technique, an eval methodology with numbers, a context-compaction recipe, a named failure mode and its fix) — never generic ("use good prompts")
4. **Write** a comprehensive summary focusing on what's most useful for engineers building agents
...
[
  {{
    ...
    "topic": "TOPIC_KEY",
    "key_insights": [
      "First key insight with a specific technique, pattern, or benchmark number",
      "Second key insight",
      "Third key insight"
    ],
...
## Rules
- `id` format: lowercase source name (no spaces, use hyphens) + date, e.g., `anthropic-2026-03-15`
- `published_date`: use YYYY-MM-DD format. If unsure, use the year and month with day 01
- `topic`: use the TOPIC_KEY provided below
- `key_insights`: 3-5 bullets, each a specific actionable insight (cite pattern names, eval metrics, token/latency numbers, framework versions, named failure modes)
- `summary`: 3-5 paragraphs, deeply covering the most important and useful parts
- Prefer articles with: real production agent case studies, post-mortems, named trade-offs, eval methodologies with numbers, context-window techniques, tool-design recipes, contrarian takes from experienced practitioners
- Skip listicles ("10 best AI agent frameworks") and surface-level marketing posts
...
def _get_harness_research_dir() -> Path:
    """Find skillnir's own .data/harness-research/ directory."""
    current = Path(__file__).resolve().parent
    for _ in range(5):
        candidate = current / ".data" / "harness-research"
        if candidate.parent.is_dir():
            candidate.mkdir(parents=True, exist_ok=True)
            (candidate / "articles").mkdir(exist_ok=True)
            return candidate
        current = current.parent
    fallback = Path.cwd() / ".data" / "harness-research"
...
def _load_index(research_dir: Path) -> dict[str, Article]:
    """Load existing harness-research-index.json for dedup."""
    index_file = research_dir / "harness-research-index.json"
...
def _generate_landing_html(
    ...
    subtitle = (
        f"{total} harness-engineering articles | Last updated: {last_updated}"
        f" | Generated: {generated_at}"
    )

    template = _load_template("landing.html")
    page_html = (
        template.replace("<!-- TITLE -->", "Harness Engineering Research")
...
# ---------------------------------------------------------------------------
# AI response parser (same shape as siblings — duplicated for module isolation)
# ---------------------------------------------------------------------------
...
def _research_topic_subprocess(
    ...
    """Search and summarize articles for a single harness topic via subprocess."""
    ...
    if sources:
        ...
        source_instruction = (
            ...
            "When searching, prioritize articles from these high-signal AI agent / LLM engineering sources. "
    ...
    prompt = _HARNESS_RESEARCH_PROMPT.format(
        topic_key=topic_key,
        search_query=search_query,
    ...
    if not articles:
        ...
        research_dir = _get_harness_research_dir()
...
async def harness_research(
    ...
    """Search the web for each harness topic, summarize with AI, generate landing page."""
    _emit(on_progress, "phase", "Initializing harness-research pipeline...")
    ...
    _emit(
        on_progress,
        "status",
        f"Loaded {len(existing)} existing harness-engineering articles for dedup",
    )
    ...
    _emit(on_progress, "phase", "Harness-research complete.")
