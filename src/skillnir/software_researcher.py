"""Software-engineering research pipeline: search the web for SE / architecture / craftsmanship news, deep-read articles, summarize, generate landing page.

Sibling of :mod:`skillnir.researcher` (AI engineering) and
...
prompt focus differ. Designed so the three domains can evolve
independently; a future refactor can extract a shared engine.
"""
...
@dataclass
class Article:
    """A single software-engineering research article with deep summary."""
    ...
    topic: str  # architecture, ddd, refactoring, distributed-systems, etc.
    key_insights: list[str] = field(default_factory=list)
    summary: str = ""


@dataclass
class SoftwareResearchResult:
    """Result of a software-engineering research pipeline run."""
...
# ---------------------------------------------------------------------------
# Topics — 14 software-engineering topic queries
# ---------------------------------------------------------------------------

RESEARCH_TOPICS: tuple[str, ...] = (
    "software architecture microservices monoliths modular hexagonal clean architecture 2025 2026",
    "domain-driven design DDD bounded contexts event storming aggregates 2025 2026",
    "refactoring techniques code smells legacy modernization strangler fig 2025 2026",
    "distributed systems consensus consistency Raft Paxos CRDT linearizability 2025 2026",
    "event-driven architecture Kafka Pulsar event sourcing CQRS sagas 2025 2026",
    "API design REST GraphQL gRPC versioning OpenAPI contract-first 2025 2026",
    "concurrency async patterns actor model channels coroutines structured concurrency 2025 2026",
    "database design data modeling SQL NoSQL vector indexing partitioning 2025 2026",
    "performance engineering profiling latency throughput observability flame graphs 2025 2026",
    "code quality SOLID clean code technical debt maintainability cyclomatic complexity 2025 2026",
    "software design patterns GoF modern functional reactive idiomatic 2025 2026",
    "software craftsmanship code review pair programming mob programming trunk-based 2025 2026",
    "developer experience DX platform engineering internal developer platform Backstage 2025 2026",
    "software documentation ADR architecture decision records C4 diagrams as code 2025 2026",
)

TOPIC_LABELS: dict[str, str] = {
    "architecture": "Software Architecture",
    "ddd": "Domain-Driven Design",
    "refactoring": "Refactoring & Modernization",
    "distributed-systems": "Distributed Systems",
    "event-driven": "Event-Driven Architecture",
    "api-design": "API Design",
    "concurrency": "Concurrency & Async",
    "data-modeling": "Database & Data Modeling",
    "performance": "Performance Engineering",
    "code-quality": "Code Quality & Tech Debt",
    "design-patterns": "Design Patterns",
    "craftsmanship": "Software Craftsmanship",
...
# ---------------------------------------------------------------------------
# Preferred sources — high-signal software-engineering domains
# ---------------------------------------------------------------------------
...
SOURCE_FILTERS: dict[str, str] = {
    "thought-leaders": "Thought Leaders (Fowler, Uncle Bob, Hillel Wayne, dan luu)",
    "consultancies": "Consultancies (ThoughtWorks)",
    "engineering-blogs": "Big-Tech Engineering Blogs (Netflix, Uber, Stripe, Spotify, LinkedIn, Meta, Lyft)",
    "platform-blogs": "Platform Blogs (GitHub, Cloudflare, AWS, Microsoft, Shopify, Slack, Dropbox, Discord)",
    "communities": "Communities & Magazines (InfoQ, DZone, High Scalability, Increment, ACM Queue)",
    "newsletters": "Newsletters & Q&A (StackOverflow Blog, The New Stack)",
    "distributed-research": "Distributed Systems Research (Aphyr, Jepsen)",
}
...
SOURCE_DOMAINS: dict[str, tuple[str, ...]] = {
    ...
    "consultancies": ("thoughtworks.com",),
    "engineering-blogs": (
        "netflixtechblog.com",
        "eng.uber.com",
        "stripe.com",
...
_TOPIC_COLORS: dict[str, str] = {
    ...
    "api-design": "#d97706",
    "concurrency": "#dc2626",
    "data-modeling": "#0d9488",
    "performance": "#9333ea",
    "code-quality": "#0284c7",
    "design-patterns": "#65a30d",
    "craftsmanship": "#be123c",
    "developer-experience": "#475569",
...
_SOFTWARE_RESEARCH_PROMPT = """\
You are a deep research assistant focused on **software engineering, architecture, and craft**. \
Your task is to search the internet for the latest articles, talks, framework releases, \
case studies, and research papers on software-engineering topics, then deeply read each one \
and extract the most important, actionable insights for backend / platform / staff engineers, \
...
1. **Search** for recent, high-quality articles (engineering blogs, architecture posts, conference talks, design-pattern essays, post-mortems, vendor case studies, ADRs)
2. **Deep-read** each article — do not just paraphrase the title
3. **Extract** 3-5 KEY INSIGHTS that are actionable and specific (e.g., a pattern with a concrete example, a benchmark number, a refactor recipe, a trade-off the author articulated) — never generic ("write good code")
4. **Write** a comprehensive summary focusing on what's most useful for working engineers and architects
...
[
  {{
    ...
    "topic": "TOPIC_KEY",
    "key_insights": [
      "First key insight with specific data, technique, or pattern name",
      "Second key insight",
      "Third key insight"
    ],
...
## Rules
- `id` format: lowercase source name (no spaces, use hyphens) + date, e.g., `martinfowler-2026-03-15`
- `published_date`: use YYYY-MM-DD format. If unsure, use the year and month with day 01
- `topic`: use the TOPIC_KEY provided below
- `key_insights`: 3-5 bullets, each a specific actionable insight (cite pattern names, benchmark numbers, library versions, named trade-offs)
- `summary`: 3-5 paragraphs, deeply covering the most important and useful parts
- Prefer articles with: real production case studies, post-mortems, named trade-offs, refactor recipes, benchmark numbers, architecture decision records, contrarian takes from experienced engineers
- Skip listicles ("10 best design patterns") and surface-level marketing posts
...
def _get_software_research_dir() -> Path:
    """Find skillnir's own .data/software-research/ directory."""
    current = Path(__file__).resolve().parent
    for _ in range(5):
        candidate = current / ".data" / "software-research"
        if candidate.parent.is_dir():
            candidate.mkdir(parents=True, exist_ok=True)
            (candidate / "articles").mkdir(exist_ok=True)
            return candidate
        current = current.parent
    fallback = Path.cwd() / ".data" / "software-research"
...
def _load_index(research_dir: Path) -> dict[str, Article]:
    """Load existing software-research-index.json for dedup."""
    index_file = research_dir / "software-research-index.json"
...
def _generate_landing_html(
    ...
    subtitle = (
        f"{total} software-engineering articles | Last updated: {last_updated}"
        f" | Generated: {generated_at}"
    )

    template = _load_template("landing.html")
    page_html = (
        template.replace("<!-- TITLE -->", "Software Engineering Research")
...
def _research_topic_subprocess(
    ...
    """Search and summarize articles for a single SE topic via subprocess."""
    ...
    if sources:
        ...
        source_instruction = (
            ...
            "When searching, prioritize articles from these high-signal software-engineering sources. "
    ...
    prompt = _SOFTWARE_RESEARCH_PROMPT.format(
        topic_key=topic_key,
        search_query=search_query,
    ...
    if not articles:
        ...
        research_dir = _get_software_research_dir()
...
async def software_research(
    ...
    """Search the web for each SE topic, summarize with AI, generate landing page."""
    _emit(on_progress, "phase", "Initializing software-research pipeline...")
    ...
    _emit(
        on_progress,
        "status",
        f"Loaded {len(existing)} existing software-engineering articles for dedup",
    )
    ...
    _emit(on_progress, "phase", "Software-research complete.")
