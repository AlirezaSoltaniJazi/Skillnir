"""Harness-engineering research page (sibling of /research and /software-research)."""
...
async def page_harness_research():
    ...
    with ui.column().classes('w-full max-w-5xl mx-auto px-8 py-8 gap-6'):
        page_header(
            t('pages.harness_research.title', lang),
            t('pages.harness_research.subtitle', lang),
            icon='hub',
        )
        ...
        with ui.row().classes('items-center gap-3'):
            ui.label(t('pages.harness_research.topics_to_search', lang)).classes(
        ...
        # ── Source chips ──
        with ui.row().classes('items-center gap-3'):
            ui.label(t('pages.harness_research.filter_by_source', lang)).classes(
                'text-sm font-medium text-secondary'
            )
        ...
        def _cache_bust_url() -> str:
            return f'/harness-research-files/index.html?t={int(time.time())}'
        ...
        if existing:
            with ui.row().classes('gap-4 flex-wrap'):
                ...
                stat_card(
                    str(active_count),
                    t('pages.harness_research.articles_in_kb', lang),
                    icon='article',
                    color='info',
                )
                if outdated_count:
            ...
            if index_path.exists():
                ...
                with ui.row().classes('gap-3'):
                    ...
                    def view_landing():
                        from skillnir.harness_researcher import regenerate_landing

                        count, _ = regenerate_landing(research_dir)
                        if count:
                            ui.notify(
                                t(
                                    'pages.harness_research.generated_html_pages',
                                    lang,
                                    count=str(count),
                                ),
                                type='positive',
                    ...
                    def generate_html_files():
                        ...
                        if count:
                            ...
                            ui.notify(
                                t(
                                    'pages.harness_research.all_articles_have_html',
                                    lang,
                                ),
                                type='info',
                            )
        ...
        async def do_research():
            ...
            if not selected_topics:
                ui.notify(
                    t('pages.harness_research.select_at_least_one_topic', lang),
                    type='warning',
                )
                return
            ...
            if result.success:
                grid = {
                    t('pages.harness_research.grid_articles_found', lang): str(
                        result.articles_found
                    ),
                    t('pages.harness_research.grid_new_articles', lang): str(
                        result.articles_new
                    ),
                    t('pages.harness_research.grid_skipped_dedup', lang): str(
                        result.articles_skipped
                    ),
                    t('pages.harness_research.grid_duration', lang): format_duration(
                        secs
                    ),
                }
                result_card(
                    results_container,
                    True,
                    t('pages.harness_research.result_success_title', lang),
                    grid_data=grid,
                )
                ...
                result_card(
                    results_container,
                    False,
                    t('pages.harness_research.result_fail_title', lang),
                    error=result.error,
                )
            ...
            with results_container:
                with ui.row().classes('gap-3 mt-4'):
                    ui.button(
                        ...
                        on_click=lambda: ui.navigate.to('/harness-research'),
                        icon='refresh',
            ...
            play_notification(audio_el, sound_state, title="Harness research complete")
