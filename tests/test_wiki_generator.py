...
class TestSnapshotWiki:
    def test_empty_when_no_files(self, tmp_path: Path):
        assert _snapshot_wiki(tmp_path) == {}

    def test_includes_llms_txt(self, tmp_path: Path):
        (tmp_path / "llms.txt").write_text("# project")
        assert set(_snapshot_wiki(tmp_path)) == {tmp_path / "llms.txt"}
...
class TestCheckWikiOutputs:
    def test_success_when_llms_and_docs_present(self, tmp_path: Path):
        ...
        result = _check_wiki_outputs(tmp_path, {}, AIBackend.CLAUDE)
    ...
    def test_files_created_diffs_against_before_snapshot(self, tmp_path: Path):
        (tmp_path / "docs").mkdir()
        old = tmp_path / "docs" / "old.md"
        old.write_text("# old")
        before = _snapshot_wiki(tmp_path)
    ...
    def test_files_created_includes_in_place_updates(self, tmp_path: Path):
        (tmp_path / "llms.txt").write_text("# x")
        (tmp_path / "docs").mkdir()
        page = tmp_path / "docs" / "architecture.md"
        page.write_text("# arch v1")
        before = _snapshot_wiki(tmp_path)

        page.write_text("# arch v2 — regenerated with more content")

        result = _check_wiki_outputs(tmp_path, before, AIBackend.CLAUDE)
        assert result.success is True
        assert page in result.files_created
        assert tmp_path / "llms.txt" not in result.files_created
